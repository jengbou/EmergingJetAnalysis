...
private:
    ...
    // outputs
    ////////////////////////////////////////
    std::unique_ptr< reco::PFJetCollection > selectedJets_;
...
JetFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    std::unique_ptr< reco::PFJetCollection > selectedJets_( new reco::PFJetCollection() );
        ...
        for (auto it = jetCollection->begin(); it != jetCollection->end(); it++) {
            ...
            if (nJetsPassing < nCuts_) {
                if (fabs(jet.eta()) < maxEtas_[nJetsPassing] && jet.pt() > minPts_[nJetsPassing]) {
                    ...
                    if (passStringCut) {
                        selectedJets_->push_back(std::move(jet));
    ...
    iEvent.put(std::move(selectedJets_), "selectedJets");

    // std::unique_ptr< double > _dummy( new double (0.0) );
